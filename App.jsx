...
function EF({expenses:ex,setExpenses:se,currencies:cc,sa,ssa}){const[n,sn]=useState({date:"",currency:"COP",category:"transporte",amount:"",obs:""});const add=()=>{if(!n.date||!n.amount)return;se(p=>[...p,{...n,id:Date.now(),amount:parseFloat(n.amount),hasReceipt:false,attachments:[]}]);sn({date:"",currency:n.currency,category:"transporte",amount:"",obs:""});ssa(false);};const tot=ex.reduce((s,e)=>s+cv(e.amount,e.currency,cc),0);
  return<div style={{...S.card,padding:18,marginBottom:14}}><div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:14}}><div style={{fontSize:14,fontWeight:700,display:"flex",alignItems:"center",gap:6}}><Receipt size={16} color={T.green}/> Gastos ({ex.length})</div><button onClick={()=>ssa(true)} style={S.btn(T.greenL)}><Plus size={14}/> Agregar</button></div>
    {sa&&<div style={{background:T.g0,borderRadius:T.rs,padding:14,marginBottom:14,border:"1px solid "+T.g2}}><div style={{display:"flex",flexDirection:"column",gap:10}}><div style={{display:"grid",gridTemplateColumns:"1fr 90px",gap:8}}><input type="date" value={n.date} onChange={e=>sn({...n,date:e.target.value})} style={{...S.inp,fontSize:13}}/><select value={n.currency} onChange={e=>sn({...n,currency:e.target.value})} style={{...S.inp,fontSize:13}}>{cc.map(c=><option key={c.code} value={c.code}>{c.code}</option>)}</select></div><select value={n.category} onChange={e=>sn({...n,category:e.target.value})} style={{...S.inp,fontSize:13}}>{CATS.map(c=><option key={c.id} value={c.id}>{c.icon} {c.label}</option>)}</select><input type="number" value={n.amount} onChange={e=>sn({...n,amount:e.target.value})} placeholder="Monto" style={{...S.inp,fontSize:13}}/><input value={n.obs} onChange={e=>sn({...n,obs:e.target.value})} placeholder="Descripcion" style={{...S.inp,fontSize:13}}/><div style={{display:"flex",gap:8}}><button onClick={add} style={{...S.btn(T.green),flex:1,padding:11}}>Agregar</button><button onClick={()=>ssa(false)} style={{...S.btn(T.g1,T.g6),padding:"11px 18px"}}>Cancelar</button></div></div></div>}
    {ex.map((e,i)=>{const cat=CATS.find(c=>c.id===e.category);const cop=cv(e.amount,e.currency,cc);return<div key={e.id} style={{display:"flex",alignItems:"center",gap:10,padding:"10px 0",borderBottom:i<ex.length-1?"1px solid "+T.g1:"none"}}><div style={{width:36,height:36,borderRadius:T.rs,background:(cat?.color||T.g5)+"12",display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0,fontSize:16}}>{cat?.icon}</div><div style={{flex:1,minWidth:0}}><div style={{fontSize:13,fontWeight:600,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{e.obs||cat?.label}</div><div style={{fontSize:11,color:T.g4,marginTop:2}}>{fs(e.date)}</div></div><div style={{fontSize:13,fontWeight:700,color:T.green,flexShrink:0}}>{fc(cop)}</div><button onClick={()=>se(p=>p.filter(x=>x.id!==e.id))} style={{...S.gh,color:T.red}}><Trash2 size={15}/></button></div>;})}
...
function Edit({rpt,cc,onX,onS}){const[f,sf]=useState({destination:rpt.destination,tripPurpose:rpt.tripPurpose,dateFrom:rpt.dateFrom,dateTo:rpt.dateTo,currency:rpt.currency,costCenter:rpt.costCenter,type:rpt.type});const[ex,se]=useState([...rpt.expenses]);const[sa,ssa]=useState(false);
  return<div className="fi" style={{maxWidth:600,margin:"0 auto"}}><button onClick={onX} style={{...S.gh,color:T.g5,gap:4,fontSize:13,fontWeight:600,marginBottom:14}}><ChevronLeft size={18}/> Cancelar</button><h1 style={{fontSize:22,fontWeight:800,marginBottom:4,letterSpacing:"-0.02em"}}>Editar {rpt.id}</h1><p style={{fontSize:13,color:T.g4,marginBottom:16}}>Se puede editar hasta la aprobacion final</p>
